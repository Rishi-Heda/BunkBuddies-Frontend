...
export default function Landing() {
    ...
    useEffect(() => {
        ...
        const loadSession = async () => {
            try {
                ...
                const nextRoute = payload?.nextRoute || (authenticated ? "/find-buddies" : "/signin");
                const shouldGoExplore = Boolean(payload?.shouldGoExplore);
                ...
                if (authenticated && shouldGoExplore) {
    ...
    const handlePrimaryAction = () => {
        if (session.authenticated) {
            router.push(session.nextRoute || "/find-buddies");
            return;
        }
    ...
    const navButtonLabel = !session.checked
        ? "Checking..."
        : session.authenticated
            ? (session.shouldGoExplore ? "Explore Rooms" : "Continue")
            : "Sign In";

    const heroButtonLabel = !session.checked
        ? "Checking session..."
        : session.authenticated
            ? (session.shouldGoExplore ? "Go to Explore Rooms" : "Continue to BunkBuddies")
            : "Find my BunkBuddy";
        ...
        <BackgroundGrid>
            <div className="min-h-screen flex flex-col overflow-hidden">
                <nav className="flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4 md:px-7">
                    ...
                    <CustomButton color="#BE8EF8" onClick={handlePrimaryAction}>
                        <svg
                            width="14"
                            height="14"
                            viewBox="0 0 24 24"
                ...
                <section className="flex flex-col items-center text-center px-4 pt-8 sm:px-6 sm:pt-10 md:px-8">
                    <div className="w-full flex flex-col items-center">
                        ...
                        <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-3 mb-8 sm:mb-10">
                            <CustomButton color="#47D19D" className="font-bold w-full sm:w-auto" onClick={handlePrimaryAction}>
                                {heroButtonLabel}
                            </CustomButton>
